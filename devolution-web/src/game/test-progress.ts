...
function getUserOperationsAtLevel(progressLevel: number) {
  ...
  const applicableActions = BASIC_INCOME_ACTIONS.filter(
    (action) => progressLevel >= action.minLevel,
  );
  applicableActions.forEach((action) => (incomeActionLevels[action.id] = 0));
  ...
  while (iterations < 100) {
    if (coinsPerSecond.equals(0)) {
      ...
      const amountToReach = getUpgradeCost(applicableActions[0], 1, progressLevel);
